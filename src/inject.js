(function () {
  ...
  // SVG icon for SWCUI (grid/apps)
  const SWCUI_ICON_SVG = `
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
  ...
  function createSwcuiNavItem() {
    const item = document.createElement("a");
    item.id = "better-gateway-swcui-nav";
    item.href = "#swcui";
    item.className = "nav-item";
    item.title = "Skills UI - Spun Web Claw (click for split view, Shift+click for SWCUI only)";
    item.innerHTML = `
      <span class="nav-item__icon" aria-hidden="true">${SWCUI_ICON_SVG}</span>
      <span class="nav-item__text">Skills</span>
    `;

    item.addEventListener("click", function (e) {
      e.preventDefault();
      if (e.shiftKey) {
        // Shift+click toggles split-swcui view (swcui + chat)
        if (currentViewMode === 'split-swcui') {
          setViewMode('chat');
        } else {
          setViewMode('split-swcui');
        }
      } else {
        // Regular click toggles SWCUI-only view (no chat sidebar by default)
        if (currentViewMode === 'swcui') {
          setViewMode('chat');
        } else {
          setViewMode('swcui');
        }
  ...
  function getActiveFrame() {
    ...
    var swcuiFrame = document.getElementById("better-gateway-swcui-frame");
    if (cliFrame && cliFrame.style.display !== "none") return cliFrame;
    if (swcuiFrame && swcuiFrame.style.display !== "none") return swcuiFrame;
    return ideFrame;
  }
  ...
  function setViewMode(mode) {
    ...
    let swcuiFrame = document.getElementById("better-gateway-swcui-frame");
    ...
    if (!splitWrapper) {
      ...
      splitWrapper.appendChild(swcuiFrame);
      ...
      swcuiFrame = document.getElementById("better-gateway-swcui-frame");
      splitHandle = document.getElementById("better-gateway-split-handle");
      
      // Lazy inject SWCUI frame if missing (e.g. from older version)
      if (!swcuiFrame) {
        swcuiFrame = createSwcuiFrame();
        splitWrapper.insertBefore(swcuiFrame, splitHandle);
      }
    }
    ...
    if (mode === 'chat') {
      ...
      if (swcuiNav) swcuiNav.classList.remove("active");
      ...
      // SWCUI only — Fullscreen
      hideFrames();
      showFrameFullscreen(swcuiFrame);
      splitHandle.style.display = "none";
      main.style.display = "none";
      ...
      if (ideNav) ideNav.classList.remove("active");
      if (cliNav) cliNav.classList.remove("active");
      if (swcuiNav) swcuiNav.classList.add("active");
      if (chatToggleButton) {
        chatToggleButton.style.display = "block";
        chatToggleButton.textContent = "←";
        chatToggleButton.title = "Show Chat Sidebar (Cmd/Ctrl+L)";
    ...
    } else if (mode === 'split-swcui') {
      // Split view: SWCUI (left) | handle | Chat (right)
      hideFrames();
      showFrameSplit(swcuiFrame);
      splitHandle.style.display = "block";
      main.style.display = "";
      main.style.flex = "1";
      main.style.width = "";
  ...
  function injectIdeNavItem() {
    ...
    // Check if ALL nav items already exist — only then skip entirely.
    ...
    var existingSwcuiNav = document.getElementById("better-gateway-swcui-nav");
    if (existingIdeNav && existingCliNav && existingSwcuiNav) {
      ideTabInjected = true;
      return false;
    }
    ...
    allNavItems.forEach(function (navItem) {
      // Skip Chat, IDE, CLI, SWCUI links (handled separately)
      if (navItem === chatLink
        || navItem.id === "better-gateway-ide-nav"
        || navItem.id === "better-gateway-cli-nav"
        || navItem.id === "better-gateway-swcui-nav") {
        return;
      }
    ...
    // Create and insert SWCUI nav item (if not already present)
    if (!existingSwcuiNav) {
      const swcuiNavItem = createSwcuiNavItem();
      navItems.appendChild(swcuiNavItem);
    }

    ideTabInjected = true;
    console.log("[BetterGateway] IDE + CLI + SWCUI nav items injected");
    return true;
  }
